...
def new_residual_block(inpt, output_depth, down_sample, projection=False):
    input_depth = inpt.get_shape().as_list()[3]
    if down_sample:
        filter_ = [1, 2, 2, 1]
        inpt = tf.nn.max_pool(inpt, ksize=filter_, strides=filter_, padding='SAME')

    conv1 = conv_layer(inpt, [1, 1, input_depth, output_depth/2], 1)
    conv2 = conv_layer(conv1, [3, 3, output_depth/2, output_depth], 1)
    conv3 = conv_layer(conv2, [1, 1, output_depth, output_depth], 1)
...
def resnet(inpt, n):
    ...
    for i in range(num_conv):
        with tf.variable_scope('conv2_%d' % (i + 1)):
            conv2_x = new_residual_block(layers[-1], 16, False)
            conv2 = new_residual_block(conv2_x, 16, False)
    ...
    for i in range(num_conv):
        ...
        with tf.variable_scope('conv3_%d' % (i + 1)):
            conv3_x = new_residual_block(layers[-1], 32, down_sample)
            conv3 = new_residual_block(conv3_x, 32, False)
    ...
    for i in range(num_conv):
        ...
        with tf.variable_scope('conv4_%d' % (i + 1)):
            conv4_x = new_residual_block(layers[-1], 64, down_sample)
            conv4 = new_residual_block(conv4_x, 64, False)
...
cost = -tf.reduce_sum(Y * tf.log(net))
